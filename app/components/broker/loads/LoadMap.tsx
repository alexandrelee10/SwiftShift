...
export default function BrokerLoadMap({ loadId, className = "" }: LoadMapProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/loads/${loadId}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Failed to fetch load (${res.status})`);
        }
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      try {
        ...
        const originFull = `${load.originAddress ?? ""} ${load.originCity}, ${
          load.originState
        }`;

        const destinationFull = `${load.destinationAddress ?? ""} ${
          load.destinationCity
        }, ${load.destinationState}`;

        const [originCoords, destinationCoords] = await Promise.all([
          geocodeAddress(originFull),
          geocodeAddress(destinationFull),
        ]);
        ...
        const routeGeometry = await fetchRoadRoute(
          originCoords,
          destinationCoords
        );
        ...
        addMarker(
          ...
          load.originAddress ?? "No address listed",
          "green"
        );
        ...
        addMarker(
          ...
          load.destinationAddress ?? "No address listed",
          "red"
        );
